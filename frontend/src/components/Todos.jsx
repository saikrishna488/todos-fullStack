...
const Todos = () => {
  ...
  const validate = async (id) => {
    try {
      let del = await axios.delete("http://localhost:5000/todo", {
        headers: {
  ...
  const validate2 = async (id) => {
    try {
      let del = await axios.delete("http://localhost:5000/success", {
        headers: {
          "Content-Type": "application/json"
        },
        data: {
  ...
  const completed = async (todo) => {
    try {
      let addTodo = await axios.post("http://localhost:5000/success", {
        text: todo.text
      });
      validate(todo.id);
